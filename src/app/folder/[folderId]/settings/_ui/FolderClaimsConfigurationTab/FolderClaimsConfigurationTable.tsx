...
export default function FolderClaimsConfigurationTable(props: FolderClaimsConfigurationTableProps) {
    ...
    const { jobFolderId, userId, activeClaimIds, page, rowsPerPage, onPageChange, onRowsPerPageChange } = props;
    ...
    const handleClaimsSwitch = async (claimIdsSwitched: number[], enable: boolean) => {
        ...
        for (const c of jobFolderClaims.filter(c => finalClaimIds.has(c.id))) {
        ...
        if (!lackingDependencyIds.length) {
            ...
            const result = await updateJobFolderClaimIdsForUser(jobFolderId, userId, request);
